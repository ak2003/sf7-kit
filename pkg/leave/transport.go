...
func NewHTTPServer(ctx context.Context, endpoints Endpoints, r *mux.Router) *mux.Router {
	...
	apiv1Sf7Leave.Methods("POST").Path("/typeOfLeave").Handler(httpTransport.NewServer(
		endpoints.GetDataTypeOfLeave,
		decodeGetDataTypeOfLeaveReq,
		response.EncodeJsonWithStatusCode,
	))
	...
	apiv1Sf7Leave.Methods("POST").Path("/remainingLeave").Handler(httpTransport.NewServer(
		endpoints.GetDataRemainingLeave,
		decodeGetDataRemainingLeaveReq,
		response.EncodeJsonWithStatusCode,
	))

	apiv1Sf7Leave.Methods("POST").Path("/createRequest").Handler(httpTransport.NewServer(
		endpoints.CreateLeaveRequestForm,
		decodeCreateLeaveRequestFormReq,
		response.EncodeJson,
	))
